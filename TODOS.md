...
2. **Tests for `JTLTemplate`** — `file`/`close`, `folder`, `manual_begin`/`manual_end`, `load_file`, `disable_backup`/`enable_backup`. These touch the filesystem so will need temp dirs.
